...
discretizacion = 34 #DEBE SER PAR #indica en cuantos cuadrados diferenciales va a ser dividido cada direccion del sistema de referencia
incremento_en_el_angulo = 15
...
alfa = 0
incremento = incremento_en_el_angulo
beta = alfa + incremento
a = velocidad_max_adm
operatividad = 0
resultados = []
...
while alfa <= 180:
    ...
    beta = alfa + incremento
    while beta <= 180 + alfa - incremento:

        print(round(alfa/180*100,2),"%")

        a3 = np.array([sin_d(beta)*a, -cos_d(beta)*a])
        b3 = np.array([sin_d(beta)*a + cos_d(beta)*a, -cos_d(beta)*a + sin_d(beta)*a])

        a4 = np.array([-sin_d(beta)*a, cos_d(beta)*a])
        b4 = np.array([-sin_d(beta)*a + cos_d(beta)*a, cos_d(beta)*a + sin_d(beta)*a])
        ...
        for c1 in range(0, discretizacion):
            for c2 in range(0, discretizacion):
                if (is_above(np.array([cuadrados[c1][c2].x_coor,cuadrados[c1][c2].y_coor]),a1,b1) and not(is_above(np.array([cuadrados[c1][c2].x_coor,cuadrados[c1][c2].y_coor]),a2,b2))) or (is_above(np.array([cuadrados[c1][c2].x_coor,cuadrados[c1][c2].y_coor]),a3,b3) and not(is_above(np.array([cuadrados[c1][c2].x_coor,cuadrados[c1][c2].y_coor]),a4,b4))):
        ...
        resultados.append([operatividad, alfa, beta])

        beta = beta + incremento

    alfa = alfa + incremento

#a continuacion se imprimen los resultados
print("RESULTADOS:")
max = resultados[0]
x =[]
y = []
...
print("---------------------------------------------------------------------------------------------------")
print("Las mejores posiciones de las pistas son:")
print(str(max[1]) + "° y " + str(max[2]) + "°, resultando en una operatividad de " + str(round(max[0],2)) + "%" )
print("---------------------------------------------------------------------------------------------------\n")
...
ax.plot([max[1],max[1]], [max[2],max[2]], [0,max[0]], color = 'lightcoral', linestyle='dashed')
ax.plot([max[1],max[1]], [360,max[2]], [max[0],max[0]], color = 'lightcoral', linestyle='dashed')
ax.plot([0,max[1]], [max[2],max[2]], [max[0],max[0]], color = 'lightcoral', linestyle='dashed')

ax.set_xticks([0,20,40,60,80,100,120,140,160,180])
ax.set_yticks([0,40,80,120,160,200,240,280,320,360])

ajustez = 2
ajustex = 3.5
ajustey = 7.5
ax.axes.set_zlim3d(bottom= 0 + ajustez, top=100) 
ax.axes.set_xlim3d(left= 0 + ajustex, right=180 - ajustex)
ax.axes.set_ylim3d(bottom= 0 + ajustey, top=360 - ajustey)
